...
import { getTeamsOfOrganization, IMemberNodeEntity, ITeamNodeEntity } from "./get-teams-of-organization";
import { IRepositoryOwners } from "./model/i-repository-owners";
import { IRepositoryOwnersDiff, ITeamMemberAddition, ITeamMemberRemoval } from "./model/i-repository-owners-diff";
...
async function determineDifferences(ctx: IExecutionContext, repositoryOwners: IRepositoryOwners, teamsInOrg: readonly ITeamNodeEntity[]): Promise<IRepositoryOwnersDiff> {
  ...
  const teamsAdded = teamNamesInYaml.filter(ty => !teamNamesInOrg.includes(ty));

  // Team names that are present in the org but not in the OWNERS.yaml file
  const teamsRemoved = teamNamesInOrg.filter(to => !teamNamesInYaml.includes(to));

  const membersAdditions: readonly ITeamMemberAddition[] = repositoryOwners.teams
    .map(aTeam => aTeam.members
      .filter(aMember => {
        return !teamsInOrg.some(t => t.name === aTeam.name && t.members.nodes?.some(m => m?.login === aMember));
      })
      .map(aMember => {
        return {
          memberName: aMember,
  ...
  const membersRemovals: readonly ITeamMemberRemoval[] = teamsInOrg
    ...
    .map((aTeam) => (aTeam.members.nodes as readonly IMemberNodeEntity[])
      .filter(aMember => {
        return !repositoryOwners.teams.some(t => t.name === aTeam.name && t.members.includes(aMember.login))
      })
      .map(aMember => {
        return {
